#!/usr/bin/env bash
# Restore production PostgreSQL data directory from a backup stored in GCS.
#
# Usage:
#   restore_prod_db.sh [backup-name-or-gs-uri] [--yes]
#
# Examples:
#   restore_prod_db.sh                      # restore latest backup
#   restore_prod_db.sh lablink-db_20260323_091111.tar.gz
#   restore_prod_db.sh gs://home-server-ss/lablink-backup/lablink-db_20260323_091111.tar.gz --yes
#
# Notes:
...
AUTO_RESTART_DB="${LL_AUTO_RESTART_DB:-true}"
# ──────────────────────────────────────────────────────────────────────────────
...
resolve_service_account_key() {
  ...
  local -a candidates=(
    "${PROJECT_ROOT}/env/service_accounts/gcs/gcs_sa.json"
    "${PROJECT_ROOT}/env/service_accounts/gcs_sa.json"
    "${SCRIPT_DIR}/backup_utility/service_account.json"
  )
  local key_path
...
[[ -d "${DATA_DIR}" ]] || fail "Production DB data directory not found: ${DATA_DIR}"
...
if [[ "${FORCE_RESTORE}" != "true" && "${FORCE_RESTORE}" != "1" ]]; then
  ...
  echo "WARNING: This will DELETE and replace data in: ${DATA_DIR}"
...
log "Downloading backup from GCS: ${backup_uri}"
...
expected_dir="${extract_root}/$(basename "${DATA_DIR}")"
if [[ -d "${expected_dir}" ]]; then
  source_data_dir="${expected_dir}"
else
  source_data_dir="$(find "${extract_root}" -type f -name PG_VERSION -printf '%h\n' | head -n 1 || true)"
  [[ -n "${source_data_dir}" ]] || source_data_dir="$(find "${extract_root}" -type d -name "$(basename "${DATA_DIR}")" | head -n 1 || true)"
...
# ── Stop container, swap data, restart ────────────────────────────────────────
db_was_running="false"
if [[ "${AUTO_RESTART_DB}" == "true" || "${AUTO_RESTART_DB}" == "1" ]]; then
  if docker inspect "${DB_CONTAINER}" >/dev/null 2>&1; then
    ...
    log "DB container '${DB_CONTAINER}' not found; continuing without container stop/start."
  fi
fi

log "Clearing existing production DB directory contents"
sudo find "${DATA_DIR}" -mindepth 1 -maxdepth 1 -exec rm -rf {} +

log "Restoring backup into ${DATA_DIR}"
sudo cp -a "${source_data_dir}/." "${DATA_DIR}/"

log "Setting PostgreSQL ownership (999:999)"
