...
- Raw SQL MUST use parameterized queries with `%s` placeholders:
  ```python
  # CORRECT
  cursor.execute("SELECT * FROM adpp_db.table WHERE id = %s", [user_id])

  # WRONG — SQL injection risk
  cursor.execute(f"SELECT * FROM adpp_db.table WHERE id = {user_id}")
  ```
- Always prefix tables with `adpp_db.` in raw SQL
