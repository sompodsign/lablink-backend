---
name: sync-db-schema
description: Fetch the current database schema from PostgreSQL and update the .claude/commands/db.md reference file. Use when the database schema has changed (new migrations applied), when db.md feels outdated, or periodically to keep the schema reference current.
---

# Sync Database Schema

Query the live PostgreSQL database and regenerate `.claude/commands/db.md` with the current schema.
...
### Step 5: Generate db.md

Write the updated schema to `.claude/commands/db.md` following this format:
...
- New tables found (not in previous db.md)
- Removed tables (in previous db.md but no longer in database)
