...
Complete a Jira ticket end-to-end: fetch details, create a branch, implement the fix, run quality checks, commit, and push.
...
### Step 3: Ensure Clean State and Create Branch
...
**IMPORTANT**: If there are uncommitted changes, stash them first and inform the user. If the branch already exists, ask the user how to proceed.
...
- If there were stashed changes, remind the user to `git stash pop` after switching back
