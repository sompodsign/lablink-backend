...
```sql
SELECT id, created_at FROM some_table
WHERE created_at >= DATE '2026-01-01' AND created_at < DATE '2026-02-01'
...
FROM parent_table p LEFT JOIN child_table c ON c.parent_id = p.id
GROUP BY p.id, p.title ORDER BY child_count ASC;
```
